...
Raphael.fn.g.dotchart = function (x, y, width, height, valuesx, valuesy, size, opts) {
    ...
    for (var i = 0, ii = valuesy.length; i < ii; i++) {
        ...
        sym && R[i] && series.push(this.g[sym](X, Y, R[i]).attr({fill: opts.heat ? this.g.colorValue(R[i], maxR) : ( opts.colors ? opts.colors[0] : Raphael.fn.g.colors[0] ), "fill-opacity": opts.opacity ? R[i] / max : 1, stroke: "none"}));
    }
    var covers = this.set();
    for (var i = 0, ii = valuesy.length; i < ii; i++) {
